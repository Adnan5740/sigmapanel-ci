    ...
    try:
        with get_db() as conn:
            ...
            if limit_setting:
                ...
                if count >= limit:
                    raise HTTPException(status_code=429, detail="Daily registration limit finished. Please contact us on Microsoft Teams at adnanman2026@outlook.com.")
...
async def view_proof(filename: str, request: Request):
    ...
    with get_db() as conn:
        row = conn.execute("SELECT id FROM registration_requests WHERE proof_filename=?", (safe_name,)).fetchone()
        if not row:
    ...
    return FileResponse(path, media_type=media_type, filename=safe_name, headers={"Cache-Control": "private, no-store"})
