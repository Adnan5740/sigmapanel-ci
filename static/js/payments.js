            ...
            <div class="card">
                ...
                <div class="table-wrapper">
                    <table class="fly-table">
                        <thead><tr><th>User</th><th>Payment ID</th><th>Amount</th><th>Method</th><th>Wallet / Note</th><th>Status</th><th>Actions</th></tr></thead>
                        <tbody>
                            ${rows.length ? rows.map(r => `
                                <tr>
                                    <td><strong>${r.username}</strong></td>
                                    <td><code style="font-size:11px">${r.payment_provider_id || r.id || '-'}</code></td>
                            ...
                            `).join('') : '<tr class="empty-row"><td colspan="7">No payout requests in queue</td></tr>'}
                        </tbody>
                    </table>
                </div>
            </div>`;
