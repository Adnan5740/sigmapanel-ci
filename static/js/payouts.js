const payouts = {
    async renderMyPayouts(container) {
        ...
        try {
            const [smsData, requestData, ledgerData, rateData] = await Promise.all([
            ...
            rows.forEach(sms => {
                ...
                const amount = Number(sms.profit || 0);
                if (!payoutMap[key]) {
                    payoutMap[key] = {
            ...
            <div class="card">
                ...
                <div class="payout-summary-grid">
                    ...
                    <div class="stat-card" style="margin:0"><div class="stat-card-label">Weekly / Monthly Rate</div><div class="stat-card-value" style="font-size:18px">${Number(rateData.weekly).toFixed(2)} / ${Number(rateData.monthly).toFixed(2)}</div></div>
                </div>
                <div class="filter-bar filter-bar-labeled">
                    <div class="filter-field filter-field-wide">
                        <label for="payout-search">Search</label>
                        <input type="text" class="search-input" id="payout-search" placeholder="Range or number" onkeyup="window.payouts.filterPayouts()">
                    </div>
